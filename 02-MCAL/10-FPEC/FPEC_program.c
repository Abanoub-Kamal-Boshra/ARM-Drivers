...
void FPEC_voidFlashPageErase(u8 Copy_u8PageNumber)
{
	...
	/* Write Page address
	 * 0x0800 0000 -- > is the address of the first page (page_0) */
	...
	/* EOP */
	SET_BIT(FPEC->SR,5); // reset by writing 1 on it
	CLR_BIT(FPEC->CR,1);
}
...
void FPEC_voidFlashWrite(u32 Copy_u32Address, u16* Copy_u16Data, u8 Copy_u8Length)
{
	...
	/* if Flash operation is in progress --> wait */
	while (GET_BIT(FPEC->SR,0) == 1);
	...
	for (i = 0; i< Copy_u8Length; i++)
	{
		...
		*((volatile u16*)Copy_u32Address) = Copy_u16Data[i];
		/* increase the address by 2 bytes (u16) */
		Copy_u32Address += 2 ;
